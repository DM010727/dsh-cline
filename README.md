...
- 🆓 **免费模型**：平台提供 **10 个免费模型**（如 `ali/qwen3.5-4b`、书生 `intern` 系列、`agnes` flash 系列、快手 `kat-coder` 系列等），0 元即可上手体验
...
- 模型选择器新增**免费模型分组**（置顶、不与厂商组重复），按平台定价自动识别，选项带「免费」标记；免费模型 0 元即可上手。

### 4.2.3

- 修复胜算云 OAuth 登录回调偶发「缺少 code 参数」的问题。
- 新增**胜算云余额/消耗卡片**：模型页直接查看账户余额与近 7 日消耗明细，支持一键刷新。

### 4.2.2

- 新用户安装引导新增**国内镜像安装**选项（npmmirror），未翻墙用户不再卡在 DSH 全局安装步骤。

### 4.2.1

- 修复端口占用（EADDRINUSE）导致 DSH 无法启动的问题：新增「清理残留 DSH 进程」命令、只绑定本扩展自己的实例、端口被占时快速报错并给出恢复指引。

### 4.2.0

- 新增**胜算云 OAuth 授权登录**：浏览器一键授权自动填入 API Key（手动粘贴仍可用，二选一）。

### 4.1.8

- 修复活动栏图标显示为纯白的问题（换用单色侧栏专用 logo）。
- 修复编辑 diff 偶发空白/不弹出的问题（写入与读取竞态）；新增「重新打开最近一次变更 diff」命令。

### 4.1.7

- 修复 macOS 粘贴内容显示空白的问题（React 受控输入框兼容）。

### 4.1.6

- 修复 macOS 下 DSH 输入框无法使用 Cmd+C/V/X/A 的问题（剪贴板桥接）；新增右键粘贴/复制/剪切/全选菜单。

### 4.1.5

- 修复 macOS 兼容性：商店描述乱码、剪贴板快捷键失效。

### 4.1.4

- 首个商店版本：DSH Agent 内核 + Cline 式 VS Code 体验（实时 diff、选中代码动作、`@` 上下文引用、终端常驻、MCP、Checkpoint）。
