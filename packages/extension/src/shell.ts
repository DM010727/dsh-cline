...
export function buildShellHtml(status: SidecarStatus, version = ''): string {
  ...
  return [
    ...
    '      return;',
    '    }',
    '    if (data.channel === "dsh-cline.shell" && data.type === "clipboard-result" && data.op === "paste") {',
    '      insertIntoFrame(String(data.text || ""));',
    '      return;',
    '    }',
    '  });',
    // Clipboard: on macOS (and Linux) VS Code webviews swallow Cmd/Ctrl+C/V/X/A
    // for an embedded <iframe>, so native shortcuts never reach the DSH app's
    // inputs. Delegate to the extension host's vscode.env.clipboard (always
    // works) instead: capture the op here, do the write/read there, and for
    // paste insert the returned text back into the iframe's focused input.
    '  function frameDoc() {',
    '    var frame = document.getElementById("dsh-frame");',
    '    return (frame && frame.contentDocument) ? frame.contentDocument : null;',
    '  }',
    '  function frameFocus(doc) {',
    '    if (!doc) return false;',
    '    try { return doc.hasFocus ? doc.hasFocus() : false; } catch (e) { return false; }',
    '  }',
    '  function activeInput(doc) {',
    '    if (!doc || !doc.activeElement || doc.activeElement === doc.body) return null;',
    '    var el = doc.activeElement;',
    '    return (el.tagName === "INPUT" || el.tagName === "TEXTAREA") ? el : null;',
    '  }',
    '  function insertIntoFrame(text) {',
    '    var doc = frameDoc(); if (!doc) return;',
    '    var el = activeInput(doc);',
    '    if (el) {',
    '      el.focus();',
    ...
    '    } else {',
    '      try { doc.execCommand("insertText", false, text); } catch (err) {}',
    '    }',
    '  }',
    ...
    '    var doc = frameDoc(); if (!doc || !frameFocus(doc)) return;',
    '    ev.preventDefault(); ev.stopPropagation();',
    '    if (key === "a") { try { doc.execCommand("selectAll"); } catch (err) {} return; }',
    '    if (key === "c" || key === "x") {',
    '      var sel = doc.defaultView.getSelection().toString();',
    '      vscode.postMessage({ channel: "dsh-cline.shell", type: "clipboard", op: key === "c" ? "copy" : "cut", text: sel });',
    '      if (key === "x") {',
    '        var el = activeInput(doc);',
    '        if (el) { var s2 = el.selectionStart, e2 = el.selectionEnd; if (s2 !== null && e2 !== null) { el.value = el.value.slice(0, s2) + el.value.slice(e2); el.setSelectionRange(s2, s2); } }',
    '        else { var sel2 = doc.defaultView.getSelection(); if (sel2 && sel2.rangeCount) sel2.deleteFromDocument(); }',
    '      }',
    '      return;',
    '    }',
    '    vscode.postMessage({ channel: "dsh-cline.shell", type: "clipboard", op: "paste" });',
    '  }, true);',
