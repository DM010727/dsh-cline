 ...
 */
function readSsyApiKey() {
    const home = process.env.DSH_HOME;
    if (home === undefined || home === '')
        return undefined;
    try {
        const file = join(home, '.credentials.yaml');
        const doc = createRequire(import.meta.url)('yaml')
            .parse(readFileSync(file, 'utf8'));
        if (doc === null || typeof doc !== 'object' || Array.isArray(doc))
            return undefined;
        const value = doc[SSY_KEY_REF];
...
async function ssyApiGet(endpoint, apiKey) {
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const [info, rate, usage] = await Promise.all([
                    ssyApiGet('/user/info', apiKey),
                    ssyApiGet('/base/rate', apiKey),
                    ssyApiGet('/modelrouter/userlog?page=1&pageSize=1000&' + range, apiKey),
                ]);
