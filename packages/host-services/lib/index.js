 ...
 */
function bridgeFileUrl() {
    const home = process.env.DSH_HOME;
    if (home === undefined || home === '')
        return undefined;
    try {
        const parsed = JSON.parse(readFileSync(join(home, 'bridge.json'), 'utf8'));
...
export async function apply(ctx, config) {
    ...
    if (!/^http:\/\/127\.0\.0\.1:\d+\/.+$/.test(bridgeUrl)) {
        throw new Error('dsh-cline-host-services: bridgeUrl must be a loopback URL with a token path '
    ...
    const service = {
        ...
        call(service, method, args) {
            return bridge({ service, method, args }).then(v => v);
        },
    };
...
function createBridgeClient(resolveUrl) {
