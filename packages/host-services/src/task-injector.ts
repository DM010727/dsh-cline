...
function sessionIdOf(subject: unknown): string | undefined {
  if (subject === null || typeof subject !== 'object') return undefined
  const s = subject as { id?: unknown; sessionId?: unknown }
  const id = s.id ?? s.sessionId
  return typeof id === 'string' && id !== '' ? id : undefined
}

/** Track the active session from DSH's session/turn/user events (global: true). */
function trackActiveSession(ctx: Context): void {
  const events = ctx as unknown as {
    on(name: string, listener: (...args: any[]) => any, options?: unknown): unknown
  }
  const g = { global: true } as const
  events.on('session/event', (subject: unknown, _event: unknown) => {
    const id = sessionIdOf(subject)
    if (id !== undefined) currentSessionId = id
  }, g)
  events.on('turn/start', (subject: unknown) => {
    const id = sessionIdOf(subject)
    if (id !== undefined) currentSessionId = id
  }, g)
  events.on('user/message', (subject: unknown) => {
    const id = sessionIdOf(subject)
    if (id !== undefined) currentSessionId = id
  }, g)
}
 ...
 */
export function registerTaskInjector(ctx: Context, webServer: WebServerService): void {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req: import('node:http').IncomingMessage, res: import('node:http').ServerResponse) => {
      ...
      let body: { content?: unknown; sessionId?: unknown; cwd?: unknown }
      try {
        body = await readJson(req) as { content?: unknown; sessionId?: unknown; cwd?: unknown }
      } catch (err: unknown) {
      ...
      const sessionId = typeof body.sessionId === 'string' && body.sessionId !== ''
        ? body.sessionId
        : currentSessionId
      const agents = ctx.get('agents') as { get(id: string): AgentLike | undefined; create(options: unknown): Promise<{ agent: AgentLike }> } | undefined
...
/** Minimal live-agent surface: what the injection needs. */
interface AgentLike {
  followup(message: unknown): void
}

/** Create a fresh agent + session and inject the prompt, mirroring dsh-headless. */
async function createTaskSession(ctx: Context, content: string, cwd?: unknown): Promise<string | undefined> {
  const agents = ctx.get('agents') as {
    ...
    }): Promise<{ agent: AgentLike; session: { id: unknown } }>
  } | undefined
  ...
  try {
    const { agent, session } = await agents.create({
      ...
      setup: (agentCtx: unknown) => installModelSelection(agentCtx, {
        current: selection,
        assembled: undefined,
      }),
    })
    ...
    void sessions?.flush(session).catch(() => { /* persist is best-effort */ })
    return sessionId
  } catch (err: unknown) {
